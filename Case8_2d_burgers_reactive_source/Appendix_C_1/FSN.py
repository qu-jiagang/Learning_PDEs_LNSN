...
class Net(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(2, 32, 3, 1, 1, bias=False, padding_mode='circular'),
            nn.Conv2d(32, 2, 1, 1, 0, bias=False, padding_mode='circular'),
        )
        self.layer2 = nn.Sequential(
            nn.Conv2d(2, 32, 3, 1, 1, padding_mode='circular'),
            nn.GELU(),
            nn.Conv2d(32, 2, 1, 1, 0, bias=False, padding_mode='circular'),
        )
...
# Network = Net().cuda()
Network = torch.load('LNSN-1.net')
for para in Network.layer1.parameters():
...
while epoch < 1000000:
    ...
    #     loss_base = loss
    #     torch.save(Network, 'LNSN-2.net')

np.savetxt('loss_data.txt',loss_sum)
